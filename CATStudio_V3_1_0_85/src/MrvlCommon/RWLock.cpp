...
#ifdef _DEBUG_RWLOCK
   if( m_mapActive.find(nThreadID)==m_mapActive.end() )
   {
      m_mapActive[nThreadID] = 1;
   }
   else
   {
      m_mapActive[nThreadID] = m_mapActive[nThreadID]+1;
   }
#endif
...
#ifdef DEBUG_RWLOCK
   TRACE("%u %s(%p) pend %d\n", GetCurrentThreadId(), __FUNCTION__, this, fResourceWritePending);
#endif
   if (fResourceWritePending) {
...
#ifdef DEBUG_RWLOCK
   TRACE("%u %s(%p) pend %d\n", GetCurrentThreadId(), __FUNCTION__, this, fResourceOwned);
#endif
   if (fResourceOwned) {
...
VOID CRWLock::Done() {
   ...
   TRACE("%u %s(%p) wake %d\n", GetCurrentThreadId(), __FUNCTION__, this,
         hsem == m_hsemReaders ? 1 : (hsem == m_hsemWriters ? 2 : 0));
#endif
   if (hsem != NULL) {
