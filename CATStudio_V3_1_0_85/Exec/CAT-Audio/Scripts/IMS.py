...
############################# Capture Logs #########################################
#capture ICAT log here
print 'start to capture ICAT log'
cats.StartICAT()
...
#capture the ip packages log on UE
cmd = '.\\adb.exe wait-for-device shell tcpdump -p -s 0 -i any -w /data/DKBx_x.cap'
print cmd
p_tcpdump = subprocess.Popen(cmd, startupinfo=startup_info)
...
subprocess.Popen("taskkill /F /PID {pid} /T".format(pid=p_tcpdump.pid))
...
#pull ip packages log
cap_file_name = os.path.join(temp_dir, filename_prefix + '.cap')
cmd = '.\\adb.exe wait-for-device pull /data/DKBx_x.cap ' + cap_file_name
print cmd
...
#stop capturing ICAT log
print 'Stop capturing ICAT log'
cats.StopICAT()


#get ICAT log name, copy and rename it
icat_log_name = cats.GetLogName()
new_icat_log_name = os.path.join(temp_dir, filename_prefix + '.icl')
shutil.copy(icat_log_name, new_icat_log_name)
...
#zip file
default_file_name = filename_prefix + '_IMS.zip'
